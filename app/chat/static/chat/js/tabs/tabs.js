...
class RoomTab extends Tab {
    ...
    async activate() {
        ...
        // contentNode is cached (noCache=false) — bind once, or every
        // activation would stack duplicate listeners.
        if (!this.contentNode.__roomBound) {
            ...
            this.contentNode.__roomBound = true;
        }
...
export function setTabMetadata(tabId, metadata) {
    ...
    if (state.tabsById[tabId]) {
        ...
        const newType = meta.special ? String(meta.special) : (meta.type ? String(meta.type) : null);
        if (newType && newType !== oldType) {
            // Behavior lives on the instance's class, so a type change
            // requires swapping the instance — just flipping `tab.type`
            // would leave e.g. a NewTab rendering new_tab forever.
            // (This keeps the old morph-in-place callers working.)
            if (TAB_TYPES[newType]) {
                const swapped = new TAB_TYPES[newType](tab.id, tab.title, tab.metadata, newType);
                state.tabsById[tabId] = swapped;
            } else {
                tab.type = newType;
                tab.destroy();
